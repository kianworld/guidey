...
var Week = /** @class */ (function () {
    function Week(weekNumberOrDateStringOrDateObj, month) {
        if (typeof weekNumberOrDateStringOrDateObj === "string") {
            ...
            if (weekNumberOrDateStringOrDateObj < 5000) {
                if (month) {
                    // get first Thursday in year
                    var start = new Date(Date.UTC(weekNumberOrDateStringOrDateObj, 0, 1));
                    while (start.getUTCDay() !== 4)
                        start.setUTCDate(start.getUTCDate() + 1);
                    // add week - 1 to get Thursday of desired week
                    var thursdayOfInputWeek = start.getTime() + (month - 1) * 604800000;
                    // divide
                    this.weekSinceEpoch = thursdayOfInputWeek / 604800000;
                    this.weekOfYear = this.getWeekOfYear(new Date(this.weekSinceEpoch * 604800000));
                }
                else {
        ...
        this.monday = new index_1.DateWithoutTime(this.weekSinceEpoch * 7 - 3);
        this.friday = new index_1.DateWithoutTime(this.weekSinceEpoch * 7 + 1);
        this.sunday = new index_1.DateWithoutTime(this.weekSinceEpoch * 7 + 3);
    ...
    Week.prototype.date2WeekSinceEpoch = function (anyOldDate) {
        return Math.floor((anyOldDate.getTime() + 259200000) / 604800000);
    };
    Week.prototype.getWeekOfYear = function (d) {
    ...
    Week.prototype.getNumberOfWeeksInYear = function (year) {
        var month = 11, day = 31, week, d;
    ...
    Week.prototype.toString = function () {
        var weekOfYear = this.getWeekOfYear(this.monday.utcMidnightDateObj);
        return "week ".concat(weekOfYear);
    };
    Week.prototype.toLocaleStringFull = function (_a) {
        var locale = _a.locale;
        locale = locale || "en-EN";
        return "".concat(this.monday.toLocaleDateString(locale, { timeZone: "UTC" }), " - ").concat(this.friday.toLocaleDateString(locale, { timeZone: "UTC" }));
    };
    return Week;
}());
exports.Week = Week;
